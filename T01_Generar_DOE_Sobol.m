...
%
% Genera un DOE con Sobol Sequence (Low-Discrepancy) sobre 6 variables:
...
% =========================================================================
m1_min = 2;   m1_max = 4;           % Espesor de pared Viga A [mm]
a1_min = 40;  a1_max = 100;         % Altura de seccion Viga A [mm]
m2_min = 2;   m2_max = 4;           % Espesor de pared Viga B [mm]
a2_min = 40;  a2_max = 100;         % Altura de seccion Viga B [mm]
b_min  = 40;  b_max  = 100;         % Anchura COMÚN (b1=b2) [mm]
ec_min = 2;   ec_max = 4;           % Espesor cartela [mm]
...
%% 5. Sobol Sequence (6 variables activas)
% Columnas: [m1, a1, m2, a2, b, e_cartela]
p = sobolset(6, 'Skip', 1, 'Leap', 0);
...
fprintf('  Metodo            : Sobol Sequence (Low-Discrepancy)\n');
fprintf('  Material          : %s\n', material);

% 9. Visualizacion de cobertura Sobol
vars_act = table(m1, a1, m2, a2, b, e_cartela);
nombres_vars = {'m1','a1','m2','a2','b','e_c'};

figure('Name', 'Verificacion DOE Sobol - Variables activas', 'Position', [100 100 1200 700]);
...
sgtitle(sprintf('DOE Sobol - %d muestras para ANSYS', numSamples));

% Matriz de correlacion
figure('Name', 'Ortogonalidad del DOE - Sobol');
heatmap(nombres_vars, nombres_vars, round(corrcoef(vars_act{:,:}), 2));
title('Matriz de correlacion de Pearson (Sobol)');
